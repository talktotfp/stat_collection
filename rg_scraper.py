...
oracle_db = sqlalchemy.create_engine('oracle://nba:rp4490@localhost:1522/xe')
connection = oracle_db.connect()


oracle_query = "select GURU_ID, REPLACE(GURU_URL,'playrh','playrh') as GURU_URL from SCRAPING_ROTO_GURU_BASE"

url_df = pd.read_sql_query(oracle_query,oracle_con)
rg_list = url_df['GURU_URL'].tolist()


def rg_url_scraper(urls, con):
    x = 0
    y = len(urls)
    ret_df = pd.DataFrame()
    for url in urls:
        ...
        if url[29:39] == 'playrh15.c':
            season = '2014'
        guru_id = int(url[(len(url)-5):(len(url)-1)])
        ...
        if len(table_body) > 4:
            ...
            if len(rows) > 50 :
                ...
                x = x+1
                pd_data.to_sql('SCRAPE_RG_PGAME', con, flavor=None,if_exists='append', 
                               index=True, index_label='SCRAPE_RG_PGAME_ID', chunksize=100, dtype=None)
                print (x,'/',y,' - ',name, ' written to database for season ', season)
    return ret_df


new_df = rg_url_scraper(rg_list, connection)
#new_df.to_csv('rg_master_2015.csv', sep=',')
#new_df.to_sql('SCRAPE_RG_PGAME', connection, flavor=None,if_exists='append', 
                 #index=True, index_label='SCRAPE_RG_PGAME_ID', chunksize=100, dtype=None)
oracle_cur.close()
oracle_con.close()
connection.close()
